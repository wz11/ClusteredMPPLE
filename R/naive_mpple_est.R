...
naive_mpple_est <- function(data, formula1 =  y ~ x + Z1 + Z2, formula2 = Surv(x, d) ~ Z1 + Z2, cause = 1, w = FALSE, ...){
  ...
  cum_residual_process <- function(t){
    mean(data$r*((data$x == t)*(data$c == cause) - data$yhat*(data$x == t)*(data$c > 0)) / (1*(w==FALSE) + data$clustersize*(w==TRUE)))
  }

  crp <- sapply(t, cum_residual_process, simplify = TRUE)

  out <- list(beta = beta, time = time, H = H, crp = crp, t = t)
  return(out)
}
