...
naive_mpple_se_bs <- function(data, formula11 =  y ~ x + Z1 + Z2, formula12 =  y ~ x + Z1 + Z2, formula21 = Surv(x, d) ~ Z1 + Z2, formula22 = Surv(x, d) ~ Z1 + Z2, w = TRUE, nboot = 1000, ...){
  ...
  for (i in 1:nboot){
    print(i)
    set.seed(i)
    data_temp <- data[sample(1:N, N, replace = TRUE),]

    result_temp1 <- naive_mpple_est(data, formula1 =  formula11, formula2 = formula21, cause = 1, w = w)
    result_temp2 <- naive_mpple_est(data, formula1 =  formula12, formula2 = formula22, cause = 2, w = w)

    cif_temp1 <- naive_cif_est(est1 = result_temp1, est2 = result_temp2, cause = 1, x)
    cif_temp2 <- naive_cif_est(est1 = result_temp1, est2 = result_temp2, cause = 2, x)
